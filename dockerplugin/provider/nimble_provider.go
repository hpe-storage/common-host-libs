...
func AddRemoveCertContainerProvider(containerProviderURI string, ipAddress string, hostCert string, username string, password string) error {
	...
	_, err := containerProviderLoginClient.DoJSON(&connectivity.Request{Action: "POST", Path: containerProviderURI, Payload: &request, Response: nil, ResponseError: nil})
	if err != nil {
		log.Trace(err.Error())
		return err
	}
...
func checkIfHostCertsExist() bool {
	log.Trace("checkIfHostCertsExist called")
