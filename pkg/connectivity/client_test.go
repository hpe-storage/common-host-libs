...
func verifyEmpty(err error, bad badnews, t *testing.T) {
	// empty is not an error now so we shouldn't get io.EOF
	if err != nil && err == io.EOF {
		t.Error(
			"expected to get an nil response from POST to /empty io.EOF is not treated as an error",
		)
	}
	if bad.Info != "" {
		t.Error(
