...
import (
	...
	"github.com/hpe-storage/common-host-libs/pkg/connectivity"
	"github.com/hpe-storage/common-host-libs/pkg/docker/dockerlt"
	log "github.com/hpe-storage/common-host-libs/pkg/logger"
)
...
func (dvp *DockerVolumePlugin) Capabilities() (*CapResponse, error) {
	...
	err := dvp.driverRun(&connectivity.Request{
		...
		ResponseError: res})
	if err != nil {
		log.Infof("unable to get Capabilities - %s\n", err.Error())
		return nil, err
	}

	log.Tracef("returning %#v", res)
	return res, nil
}
...
func (dvp *DockerVolumePlugin) Get(name string) (*GetResponse, error) {
	...
	err := dvp.driverRun(&connectivity.Request{
		...
		ResponseError: res})
	if err != nil {
		log.Infof("unable to get docker volume using %s - %s response - %v\n", name, err.Error(), res)
		return nil, err
	}

	if err = driverErrorCheck(res); err != nil {
		log.Infof("unable to get docker volume using %s - %s\n", name, err.Error())
		return nil, err
	}
	log.Tracef("returning %#v", res)
	return res, nil
...
func (dvp *DockerVolumePlugin) List() (*GetListResponse, error) {
	...
	err := dvp.driverRun(&connectivity.Request{
		...
		ResponseError: res})
	if err != nil {
		log.Infof("unable to list docker volumes - %s response - %v\n", err.Error(), res)
		return nil, err
	}

	if err = driverErrorCheck(res); err != nil {
		log.Infof("unable to list docker volumes - %s\n", err.Error())
		return nil, err
	}
	log.Tracef("returning %#v", res)
	return res, nil
...
func (dvp *DockerVolumePlugin) createOrUpdate(name string, options map[string]interface{}, isUpdate bool) (string, error) {
	...
	if err != nil {
		log.Errorf("unable to create/update docker volume using %v & %v - %s response - %v\n", name, options, err.Error(), res)
		return "", err
	}
	if err = driverErrorCheck(res); err != nil {
		return "", err
...
func (dvp *DockerVolumePlugin) Update(name string, options map[string]interface{}) (string, error) {
	...
	if err != nil {
		log.Errorf("unable to update docker volume using %v & %v - %s\n", name, options, err.Error())
		return "", err
	}
	return name, nil
}
...
func (dvp *DockerVolumePlugin) Create(name string, options map[string]interface{}) (string, error) {
	...
	if err != nil {
		log.Errorf("unable to create docker volume using %v & %v - %s\n", name, options, err.Error())
		return "", err
	}
	return name, nil
}
...
func (dvp *DockerVolumePlugin) Mount(name, mountID string) (string, error) {
	log.Tracef("Mount called with %s %s", name, mountID)
	try := 0
	for {
		log.Tracef("dvp.mounter() called with %s %s %s try:%d", name, mountID, MountURI, try+1)
...
func (dvp *DockerVolumePlugin) Unmount(name, mountID string) error {
	log.Tracef("Unmount called with %s %s", name, mountID)
	try := 0
	for {
		log.Tracef("dvp.mounter() called with %s %s %s try:%d", name, mountID, UnmountURI, try+1)
...
func (dvp *DockerVolumePlugin) Delete(name string, managerName string) error {
	...
	err := dvp.driverRun(&connectivity.Request{
		...
		ResponseError: res})
	if err != nil {
		log.Errorf("%s failed %v - %s response - %v\n", RemoveURI, name, err.Error(), res)
		return err
	}

	if err = driverErrorCheck(res); err != nil {
		log.Errorf("%s failed %v - %s\n", RemoveURI, name, err.Error())
		return err
	}
...
func (dvp *DockerVolumePlugin) mounter(name, mountID string, path string) (string, error) {
	...
	err := dvp.driverRun(&connectivity.Request{
		...
		ResponseError: res})
	if err != nil {
		log.Errorf("%s failed %v & %v - %s response - %v\n", path, name, mountID, err.Error(), res)
		return "", err
	}

	if err = driverErrorCheck(res); err != nil {
		log.Errorf("%s failed %v & %v - %s\n", path, name, mountID, err.Error())
		return "", err
	}
