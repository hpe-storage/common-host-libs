...
func NewChapiClient(chapiFolder string) (chapiClient *Client, err error) {

	// If no CHAPI folder path is provided, using the running executable's path
	if chapiFolder == "" {
	...
	if chapiClient, err = NewChapiClientWithTimeout(chapiPortFilePath, timeout); err != nil {
...
func NewChapiClientWithTimeout(chapiPortFilePath string, timeout time.Duration) (*Client, error) {
...
func (chapiClient *Client) GetAccessKey() (string, error) {
	...
	_, err := chapiClient.client.DoJSON(&connectivity.Request{Action: "GET", Path: "/api/v1/keyfile", Payload: nil, Response: &chapiResp, ResponseError: &chapiResp})
	if errResp != nil {
		log.Trace(errResp.Text)
